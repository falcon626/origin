...
bool Application::Init(int w, int h)
{
	...
	// ウィンドウ作成
	//===================================================================
	if (KdWindow::Instance().Create(w, h, "3D GameProgramming", "Window") == false) {
	...
	// Direct3D初期化
	std::string errorMsg;
	if (KdDirect3D::Instance().Init(KdWindow::Instance().GetWndHandle(), w, h, deviceDebugMode, errorMsg) == false) {
		MessageBoxA(KdWindow::Instance().GetWndHandle(), errorMsg.c_str(), "Direct3D初期化失敗", MB_OK | MB_ICONSTOP);
		return false;
	}
	...
	if (bFullScreen) {
		...
		if (FAILED(hr))
		{
			MessageBoxA(KdWindow::Instance().GetWndHandle(), "フルスクリーン設定失敗", "Direct3D初期化失敗", MB_OK | MB_ICONSTOP);
			return false;
		}
	}

	//m_maxFps = m_fpsController.GetMaxFPS();
...
void Application::Execute()
{
	...
	while (true)
	{
		...
		// ウィンドウのメッセージを処理する
		KdWindow::Instance().ProcessMessage();

		// ウィンドウが破棄されてるならループ終了
		if (KdWindow::Instance().IsCreated() == false)
		{
			break;
		}

		if (Key::IsPushingWithFocus(Key::Delete))
		{
			if (MessageBoxA(KdWindow::Instance().GetWndHandle(), "本当にゲームを終了しますか？",
...
void Application::ImGuiInit()
{
	...
	ImGui_ImplWin32_Init(KdWindow::Instance().GetWndHandle());
	ImGui_ImplDX11_Init(
